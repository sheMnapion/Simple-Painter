...
class Parser(object):
    ...
    def drawLineAnalysis(self,lexicals):
        ...
        if algorithm not in algorithms:
            print("Unknown algorithm! We only support:",end='')
        ...
        if self.waitSecondLine is False:
            ...
            if self.waitFor=="polygon":
                ...
                if state is True:
                    # print "Accepted!"
                    self.clearWaitState()
                    panel.drawPolygon(self.waitId,points,self.waitAlgorithm)
            if self.waitFor=="curve":
                state,points=self.drawCurveSecondLineAnalysis(lexicals)
                if state is True:
                    self.clearWaitState()      
                    panel.drawCurve(self.waitId,points,self.waitAlgorithm)
        print("Query time: %.3f ms" % (1000*(time.time()-start)))  